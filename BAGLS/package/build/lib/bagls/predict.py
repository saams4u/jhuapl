...
from bagls.models.hrnet import *
...
#------------Download weights--------------------------------------
hrnet_model_name = "hrnet"
hrnet_weights = str(Path.home())+f"{os.sep}.bagls{os.sep}models{os.sep}"+ hrnet_model_name + ".pth"

resnet_model_name = "resnet50"
resnet_weights = str(Path.home())+f"{os.sep}.bagls{os.sep}models{os.sep}"+ resnet_model_name + ".pth"
...
# url = "https://dl.dropboxusercontent.com/s/wvxygz94ywvkb3f/hrnet.pth?dl=1"
url = "https://dl.dropboxusercontent.com/s/ye8o6hmdo1eaf2o/resnet50.pth?dl=1"

if  not os.path.exists(resnet_weights):
    with DownloadProgressBar(unit="B", unit_scale=True,
                             miniters=1, desc=url.split("/")[-1]) as t:
        os.makedirs(os.path.dirname(resnet_weights), exist_ok=True)
        urllib.request.urlretrieve(url, filename=resnet_weights, reporthook=t.update_to)
...
# model = torch.hub.load('pytorch/vision:v0.7.0', 'resnet50', map_location=device)
# model.load_state_dict(torch.load(resnet_weights, map_location=device))

model = torch.hub.load('pytorch/vision:v0.7.0', 'resnet50', pretrained=True)
# model.load_state_dict(torch.load(resnet_weights, map_location=device))

# model = torch.jit.load(resnet_weights)

model = model.cpu()
model.eval()

models[resnet_model_name] = model


def predict_image(input_image_path, output_folder):
    os.makedirs(output_folder, exist_ok=True)
    get_prediction_several_models(input_image_path, output_folder, [resnet_model_name])
...
def predict_folder(input_folder, output_folder):
    ...
    for image_path in images_path:    
        if "_pred." not in image_path.lower():
            get_prediction_several_models(image_path, output_folder, [resnet_model_name])